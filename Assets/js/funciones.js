let tblUsuarios, tblRoles, tblProveedores, tblProductosb, tblClientes, tblTallas, tblInventario, tblKardex,
    tblMateriaPrima, tblProduccion, tblCatalogo, tblEntregas, tbldescC;
document.addEventListener("DOMContentLoaded", function () {
    ...
    tbldescC = $('#tblrealizadasDESC').DataTable({
        ajax: {
            url: base_url + "Compras/listar_realizadasCompras",
            dataSrc: ''
        },
        columns: [
            {
        ...
        //Botón para PDF
        {
            ...
            download: 'open',
            footer: true,
            title: 'Reporte ultimas compras',
            filename: 'Reporte ultimas compras',
            text: '<span class="badge  badge-danger"><i class="fas fa-file-pdf"></i></span>',
            exportOptions: {
                columns: [0, ':visible']
            }
...
tblMateriaPrima = $('#tblMateriaPrima').DataTable({
    ...
    //Botón para PDF
    {
        ...
        filename: 'Reporte de Materia prima',
        text: '<span class="badge  badge-danger"><i class="fas fa-file-pdf"></i></span>',
        exportOptions: {
            columns: [0, ':visible']
        }
...
$('#tblvrealizadasDESC').DataTable({
    ajax: {
        url: base_url + "Ventas/listar_realizadasDESC",
        dataSrc: ''
    },
    columns: [
        {
    ...
    buttons: [
        ...
        //Botón para PDF
        {
            ...
            download: 'open',
            footer: true,
            title: 'Reporte de ultimas ventas',
            filename: 'Reporte de ultimas ventas',
            text: '<span class="badge  badge-danger"><i class="fas fa-file-pdf"></i></span>',
            exportOptions: {
                columns: [0, ':visible']
            }
...
//Tabla entregas de producto realizado
tblEntregas = $('#tblEntregas').DataTable({
    ajax: {
    ...
    //Botón para PDF
    {
        ...
        download: 'open',
        footer: true,
        title: 'Reporte de Productos para entregar',
        filename: 'Reporte de Productos para entregar',
        text: '<span class="badge  badge-danger"><i class="fas fa-file-pdf"></i></span>',
        exportOptions: {
            columns: [0, ':visible']
        }
            ...
            if (this.readyState == 4 && this.status == 200) {
                ...
                if (res == "si") {
                    Swal.fire({
                        ...
                        text: "Se ha enviado un enlace a tu correo para cambiar la contraseña y activar tu cuenta.",
                        icon: "success"
                    })        
                    frm.reset();
...
// Función para calcular el precio y enviar la compra al servidor
function calcularPrecioc() {
    const cantidadInput = document.getElementById('cantidad_compra');
    const precioInput = document.getElementById('precio_compra');
    const subTotalInput = document.getElementById('sub_total');
...
function generarCompra() {
    ...
    let validarFactura = /^(?!(\d)\1{9})\d{1,10}$/;
    let validarIsv = /^(?!.*(\d)\1{9})\d{1,3}$/;
    let validarDescuento = /^(?!.*(\d)\1{9})\d{1,3}$/;

    if (factura.value == "" || isv.value == "" || descuento.value == "" || fecha.value == "") {
        Swal.fire({
            position: 'center',
            icon: 'error',
            title: 'Todos los campos son obligatorios',
    ...
    } else if (!validarFactura.test(factura.value)) {
        Swal.fire({
            icon: "error",
            title: "Factura no válido",
            text: "La factura solo puede contener números y debe tener una longitud máxima de 5 digitos.",
        });
    } else if (!validarIsv.test(isv.value)) {
        Swal.fire({
            icon: "error",
            title: "Impuesto no válido",
            text: "El impuesto solo puede contener números para calcular el porcentaje, con una longitud máxima de 3 digitos.",
        });
    } else if (!validarDescuento.test(descuento.value)) {
        Swal.fire({
            icon: "error",
            title: "Descuento no válido",
            text: "El descuento solo puede contener números para calcular el porcentaje, con una longitud máxima de 3 digitos.",
        });
    } else {
        Swal.fire({
...
function registrarProv(e) {
    ...
    let validarNombre = /^(?!.*(\w)\1{9,})[a-zA-ZÀ-ÿ\sáéíóúÁÉÍÓÚ]{1,40}$/;
    let validarDireccion = /^(?!.*(\w)\1{10})[\w\sáéíóúÁÉÍÓÚ!@#$%^&*()-+=<>?/.,;:'"¡¿]{3,40}$/;
    let validarTelefono = /^(?!.*0000)[0-9\-]{8,20}$/;
...
function registrarProb(e) {  //ventana emergente para registrar
    ...
    let validarNombre = /^(?!.*(\w)\1{5})[a-zA-Z0-9áéíóúÁÉÍÓÚ\s]{2,40}$/;
    let validarProducto = /^(?!.*(\w)\1{5})[a-zA-ZáéíóúÁÉÍÓÚ\s]{2,20}$/;
    let validarDescripcion = /^(?!.*(\w)\1{5,})[^\n\r]{1,}$/;
...
function registrarCliente(e) {
    ...
    const telefono = document.getElementById("telefono");
    let validarCliente = /^(?!.*(\w)\1{9,})[a-zA-ZÀ-ÿ\sáéíóúÁÉÍÓÚ]{1,40}$/;
    let validarIdentificacion = /^(?!.*0000)[0-9\-]{5,20}$/;
    let validarTelefono = /^(?!.*0000)[0-9\-]{8,20}$/;
    ...
    http.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            try {
                ...
                const defaultOption = new Option('Productos', '', true, true);
                selectTipo.add(defaultOption);
...
function calcularPreciov(e) {
    ...
    const color_letra = document.getElementById("color_letra");
    const logo_izquierdo = document.getElementById("logo_izquierdo");
    const logo_derecho = document.getElementById("logo_derecho");
    const logo_delantero = document.getElementById("logo_delantero");
    const logo_trasero = document.getElementById("logo_trasero");
    let validarCantidad = /^(?!(\d)\1{3})\d{1,3}$/;
    let validarPrecio = /^(?!(\d)\1{6})\d{1,7}(?:\.\d{1,2})?$/;
    let validarColor = /^(?!.*(\w)\1{2,})[a-zA-ZáéíóúÁÉÍÓÚ0-9\s.,;:¡!¿?(){}[\]<>\-*#_$%&/]{3,70}$/;
    let validarColorLetra = /^(?!.*(\w)\1{2,})[a-zA-ZáéíóúÁÉÍÓÚ0-9\s.,;:¡!¿?(){}[\]<>\-*#_$%&/]{3,70}$/;
    let validarLogoIzquierdo = /^(?!.*(\w)\1{2,})[a-zA-ZáéíóúÁÉÍÓÚ0-9\s.,;:¡!¿?(){}[\]<>\-*#_$%&/]{3,70}$/;
    let validarLogoDerecho = /^(?!.*(\w)\1{2,})[a-zA-ZáéíóúÁÉÍÓÚ0-9\s.,;:¡!¿?(){}[\]<>\-*#_$%&/]{3,70}$/;
    let validarLogoDelantero = /^(?!.*(\w)\1{2,})[a-zA-ZáéíóúÁÉÍÓÚ0-9\s.,;:¡!¿?(){}[\]<>\-*#_$%&/]{3,70}$/;
    let validarLogoTrasero = /^(?!.*(\w)\1{2,})[a-zA-ZáéíóúÁÉÍÓÚ0-9\s.,;:¡!¿?(){}[\]<>\-*#_$%&/]{3,70}$/;

    if (cantidad.value == "" || precio.value == "" || color.value == "" || color_letra.value == "" || logo_izquierdo.value == "" || logo_derecho.value == "" || logo_delantero.value == "" || logo_trasero.value == "") {
        Swal.fire({
            position: 'center',
            icon: 'error',
            title: 'Todos los campos son obligatorios',
    ...
    } else if (!validarCantidad.test(cantidad.value)) {
        Swal.fire({
            icon: "error",
            title: "Cantidad no válida",
            text: "La cantidad solo puede contener números y debe tener una longitud máxima de 3 digitos.",
        });
    } else if (!validarPrecio.test(precio.value)) {
        Swal.fire({
            icon: "error",
            title: "Precio no válida",
            text: "El precio solo puede contener números y debe tener una longitud máxima de 7 digitos, tambien se admiten 2 dígitos despues de un punto.",
        });
    } else if (!validarColor.test(color.value)) {
        Swal.fire({
            icon: "error",
            title: "Color no válido",
            text: "El color no puede contener nombres largos ni signos.",
        });
    } else if (!validarColorLetra.test(color_letra.value)) {
        Swal.fire({
            icon: "error",
            title: "Color de letra incorrecto",
            text: "El color de letra no puede contener nombres largos ni signos.",
        });
    } else if (!validarLogoIzquierdo.test(logo_izquierdo.value)) {
        Swal.fire({
            icon: "error",
            title: "Logo izquierdo incorrecto",
            text: "El logo no puede contener nombres largos ni cortos.",
        });
    } else if (!validarLogoDerecho.test(logo_derecho.value)) {
        Swal.fire({
            icon: "error",
            title: "Logo derecho incorrecto",
            text: "El logo no puede contener nombres largos ni cortos.",
        });
    } else if (!validarLogoDelantero.test(logo_delantero.value)) {
        Swal.fire({
            icon: "error",
            title: "Logo delantero incorrecto",
            text: "El logo no puede contener nombres largos ni cortos.",
        });
    } else if (!validarLogoTrasero.test(logo_trasero.value)) {
        Swal.fire({
            icon: "error",
            title: "Logo trasero incorrecto",
            text: "El logo no puede contener nombres largos ni cortos.",
        });
    } else {
        if (cantidadInput && precioInput && subTotalInput) {
...
function generarVenta() {
    ...
    let validarDescuento = /^(?!.*(\d)\1{9})\d{1,3}$/;
    let validarAbono = /^(?!(\d)\1{6})\d{1,7}(?:\.\d{1,2})?$/;


    if (descuento.value == "" || abono.value == "") {
        Swal.fire({
            position: 'center',
            icon: 'error',
            title: 'Todos los campos son obligatorios',
    ...
    } else if (!validarAbono.test(abono.value)) {
        Swal.fire({
            icon: "error",
            title: "Abono no válido",
            text: "El abono solo puede contener números y debe tener una longitud máxima de 7 digitos, tambien se admiten 2 dígitos despues de un punto.",
        });
    } else {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                const frm = document.getElementById("frmVent");
                const frmv = document.getElementById("frmV");
                const http = new XMLHttpRequest();
                const formData = new FormData(frm);
                const clientesSelect = document.getElementById('clientes');
                const clientes = clientesSelect.options[clientesSelect.selectedIndex].text;

                for (const [key, value] of new FormData(frmv)) {
...
function handleVentaApiResponse(response) {
    if (response.msg === "ok") {
        Swal.fire({
            title: "Mensaje!",
            text: "Venta generada.",
        ...
        const ruta = base_url + 'Ventas/generarPdfv/' + response.id_venta;
        window.open(ruta);
            ...
            http.onreadystatechange = function () {
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    if (res == "ok") {
                        Swal.fire({
                            title: "Mensaje!",
                            text: "Pedido desactivado correctamente.",
                            icon: "success"
                        });
...
//Entregas
function btnDesactivarEntrega(id) {
    Swal.fire({
        title: "¿Está seguro de entregar este producto?",
        text: "¡El producto sera entregado, se eliminara de esta vista!",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
        ...
        if (result.isConfirmed) {
            const url = base_url + "Entregas/desactivar/" + id;
            const http = new XMLHttpRequest();
            http.open("GET", url, true);
            http.send();
            http.onreadystatechange = function () {
...
function preview(e) {
    // Función para subir la imagen y previsualizar
    ...
    // Asignar la URL temporal al atributo src de la imagen
    document.getElementById("img-preview").src = urlTmp;

    // Ocultar el botón de selección de imagen
    document.getElementById("icon-image").classList.add("d-none");

    // Mostrar el botón para eliminar la imagen y el nombre del archivo
...
function frmCatalogo() {
    document.getElementById("title").innerHTML = "Nueva foto";
    document.getElementById("btnAccion").innerHTML = "Registrar";
    ...
    http.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (res == "ok") {
                ...
                tblCatalogo.ajax.reload();
            } else if (res == "modificado") {
                Swal.fire({
                    position: 'center',
                    icon: 'success',
                    title: 'Imagen modificada con éxito',
                    showConfirmButton: false,
                    timer: 3000
                });
                $("#foto_catalogo").modal("hide");
...
function editarImagen(id_catalogo) {
    document.getElementById("title").innerHTML = "Imagen del Producto";
    document.getElementById("btnAccion").innerHTML = "Subir Imagen";
    const url = base_url + "Catalogo/editar/" + id_catalogo;
    const http = new XMLHttpRequest();
    http.open("GET", url, true);
    http.send();
    http.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            const res = JSON.parse(this.responseText);
            document.getElementById("id_catalogo").value = id_catalogo; // Establecer el valor del campo id_catalogo
            document.getElementById("img-preview").src = base_url + 'Assets/imagenes/' + res.foto;
            document.getElementById("icon-cerrar").innerHTML = `
            <button class="btn btn-danger" onclick="deleteImg()"><i class="fas fa-times"></i></button>`;
            document.getElementById("icon-image").classList.add("d-none");
...
function btnSubirFotocatalogo(id) {
    ...
    http.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            console.log(this.responseText); // Agregar esta línea para ver la respuesta del servidor
            const res = JSON.parse(this.responseText);
...
function btnGuardarImg(id) {
    ...
    if (imagenInput.files.length > 0) {
        ...
        const url = base_url + "Catalogo/actualizarImg/" + id;
        const http = new XMLHttpRequest();
        http.open("POST", url, true);

        // Agregar un listener para manejar la respuesta del servidor
        http.onreadystatechange = function () {
        ...
        http.send(formData); // Envía los datos del formulario al servidor
    } else {
...
function otp(e) {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        btnAccion.addEventListener('click', function () {
            const url = base_url + 'Usuarios/enviarCorreo/' + correo.value;
